...
var (
	...
	TTSModelPaths = map[ModelEntity]string{
		...
		ModelTTS_ViTTS:      filepath.Join(BaseScriptsDir, "tts", "Vi-TTS", "tts.py"),
	}
...
func (r *TTSRequest) GetModelPath() (string, error) {
	...
	if !exists {
		// Return default model path if model not found
		path, exists = TTSModelPaths[ModelTTS_ViTTS]
		if !exists {
...
func (r *LSRequest) GetModelPath() (string, error) {
	...
	if !exists {
		// Return default model path if model not found
		path, exists = LSModelPaths[ModelLS_LatentSync]
		if !exists {
			return "", fmt.Errorf("default LS model path not defined")
		}
	}
	return path, nil
}
